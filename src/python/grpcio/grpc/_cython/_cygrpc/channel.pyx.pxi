    ...
    if channel_credentials is None:
      if target.startswith("usb:"):
          ...
          if m:
            ...
            self._state.c_channel = grpc_insecure_channel_create_from_usb(<char *>target, c_vid, c_pid, channel_args.c_args())
      elif target.startswith("fd:"):
          m = re.search('fd:(.+?)', target.decode('utf8'))
          if m:
            c_fd = int(m.group(1))
            self._state.c_channel = grpc_insecure_channel_create_from_fd(<char *>target, c_fd, channel_args.c_args())
      else:
